...
export default function PipelinePage() {
  ...
  const isAnyRunning = exaState === "running" || haikuState === "running" || apolloState === "running";

  // Auto-refresh results while a flow is running
  const fetchResults = useCallback(async () => {
    if (!play) return;
    try {
      const [logsRes, enrichRes] = await Promise.all([
  ...
  }, [isAnyRunning, fetchResults]);

  // Also fetch once when a run completes
  useEffect(() => {
    if (play && (exaState === "done" || haikuState === "done" || apolloState === "done")) {
      fetchResults();
    }
  }, [exaState, haikuState, apolloState, play, fetchResults]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        />
        <FlowCard
          title="2. Haiku Screen"
          description="Apollo people search + Haiku screening"
          state={haikuState}
          runId={haikuRunId}
          error={haikuError}
        ...
        <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {stepLogs.slice(0, 100).map((log, i) => (
                  <tr key={i} className="border-b border-zinc-100 dark:border-zinc-800">
                    <td className="py-1.5 pr-4">{log.company_name}</td>
                    <td className="py-1.5 pr-4 text-zinc-400">{log.domain}</td>
                    <td className="py-1.5 pr-4">{log.step}</td>
                    ...
                    <td className="py-1.5 text-zinc-400 text-xs truncate max-w-[200px]">
      ...
      {enrichResults.length > 0 && (
        <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4">
          <h2 className="font-semibold mb-3">Enrichment Results</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-zinc-500 border-b border-zinc-200 dark:border-zinc-700">
